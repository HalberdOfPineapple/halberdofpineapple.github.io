---
permalink: /
title: "About Me"
author_profile: true
redirect_from: 
  - /about/
  - /about.html

---

I'm **Wenxuan Li**, a PhD student in Electrical and Computer Engineering at **Cornell University**, working on **efficient, scalable and reliable Machine Learning Systems** under the supervision of Dr. [Jiaxin Lin](https://www.csl.cornell.edu/~jiaxinl/).

Most recently I was a research intern at **Microsoft Research Asia** mentored by Dr. [Youshan Miao](https://www.microsoft.com/en-us/research/people/yomia/) and was awarded *Star of Tomorrow*, where I worked on building the resource-efficient performance model of dynamic MoE model training systems. Prior to this, I worked as research intern at **Microsoft Research Asia (Shanghai)**, working on efficient long-context LLM training with dynamic sparse attention under the supervision of [Chengruidong Zhang](https://openreview.net/profile?id=%7EChengruidong_Zhang1) and Dr. [Yuqing Yang](https://www.microsoft.com/en-us/research/people/yuqyang/). I also spent time at **Alibaba Cloud** as an intern, contributing to inference acceleration for [Wan 2.5](https://wan.video/).

I completed my **MPhil in Advanced Computer Science** at the **University of Cambridge** with *Distinction*, supervised by Dr. [Eiko Yoneki](https://www.cl.cam.ac.uk/~ey204/), where I worked with [Taiyi Wang](https://kevinwty0107.github.io/) on automatic DBMS configuration tuning with high-dimensional Bayesian optimization. Before that, I earned my **BSc in Artificial Intelligence and Computer Science** at the **University of Edinburgh** with *First Class Honours*, supervised by Dr. [Hao Tang](https://homepages.inf.ed.ac.uk/htang2/).

Alongside research, I contributed to [CAMEL](https://www.camel-ai.org/),the first LLM-based multi-agent framework, in its early stage under the guidance of Dr. [Guohao Li](https://ghli.org/).

## Get in touch

Feel free to reach me at [wl832@cornell.edu](mailto:wl832@cornell.edu). You can also find me on
[Google Scholar](https://scholar.google.com/citations?user=ASEeTO4AAAAJ&hl=en),
[GitHub](https://github.com/HalberdOfPineapple), and
[LinkedIn](https://www.linkedin.com/in/wenxuan-li-2415201b7/).